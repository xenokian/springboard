...
the member name. */
SELECT DISTINCT f.name, m.firstname || ' ' || m.surname as member
FROM facilities f
...
SELECT f.name, m.firstname || ' ' || m.surname as member, cost
FROM (
   SELECT b.facid, memid, slots,
   ...
   AND (CASE WHEN memid = 0 THEN slots * f.guestcost
            ELSE slots * f.membercost END) > 30
) as bookings
JOIN facilities f ON bookings.facid = f.facid
JOIN members m ON bookings.memid = m.memid
ORDER BY cost DESC;
...
SELECT f.name, COUNT(DISTINCT b.memid) as member_usage
FROM facilities f
JOIN bookings b ON f.facid = b.facid
WHERE b.memid != 0
GROUP BY f.name;
